...
# Source Alpine service functions if available
if [[ -f "/usr/local/x-ui/alpine-service-functions.sh" ]]; then
    source /usr/local/x-ui/alpine-service-functions.sh
fi
...
status() {
    if command -v service_status >/dev/null 2>&1; then
        service_status
    else
